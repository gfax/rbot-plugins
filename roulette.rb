...
class RoulettePlugin < Plugin
  ...
  def roulette(m, params)
    if m.private?
      m.reply "You gotta play roulette in a channel."
      return
    end
    playerdata = nil
    ...
    if @last == m.sourcenick and not @bot.config['roulette.twice_in_a_row']
      m.reply "I can't allow you to go twice in a row."
      return
    end
    ...
    if shot
      ...
      @players.clear
      @last = ''
      bantime = chamberNo * 60
      @bot.mode(m.replyto, "+b", m.sourceaddress) if @bot.config['roulette.kick']
      @bot.timer.add_once(bantime) { @bot.mode(m.replyto, "-b", m.sourceaddress) } if @bot.config['roulette.kick']
...
plugin.map 'reload', :action => 'reload'
plugin.map 'spin', :action => 'spin'
